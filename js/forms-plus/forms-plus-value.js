...
jQuery.extend(formsPlus, {
    ...
    */
        ...
        if( name.length ){
            switch( name.charAt(0) ){
                case '#'                                :
                    ...
                    var precision                       = false;
                    name                                = name
                        .replace(/#([0-9]+)#/g, function(_, key) {
                            precision                   = formsPlus.toNumber(key);
                            return "";
                        })
                    ;
                    if( precision === false ){
    ...
    getValue                                        : function($el, asObject, check){
        $el                                         = jQuery($el);
        if( check && (($el.is('[type="radio"], [type="checkbox"]') && !$el.is(':checked')) || ($el.is('option') && !$el.is(':selected'))) ){
