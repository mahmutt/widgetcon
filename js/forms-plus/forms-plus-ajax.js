...
if( jQuery().ajaxSubmit() ){
    var
        ...
        appendCaptchaHash                           = function($el){
            ...
            */
            var $captcha                            = $el.find('[data-js-captcha]:not([type="hidden"])');
            $captcha.each(function(i, $c){
                $c                                  = jQuery($c);
                var name                            = $c.attr('name') + 'Hash';
                $el.find('input[name="' + name + '"]').remove();
                $el.append('<input type="hidden" class="' + removeAfterSendCss + '" name="' + name +
        ...
        bindAfterActions                            = function($form, jqXHR){
            $form.find('.' + removeAfterSendCss).remove();
