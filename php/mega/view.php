<?php
				...
				*/
				...
				if($specify_format_of_data=='sequential'){//sekans formatý ise(noninterleaved)
					...
					if(preg_match('/^#/i',$line) && $sira!==0)//diez iþareti (#) ile baþlýyorsa ve ilk satýr deðilse
					{
						...
						$sekans_sayisi++;
						if(preg_match('/ /',$line)){//birey ismi ile ayný satýrda sekans verisi baþlýyorsa
					...
					{
						if(preg_match('/ /',$line)){//birey ismi ile ayný satýrda sekans verisi baþlýyorsa
							$lineArray = explode(" ", $line, 2);//Okunan satýrý ilk boþluktan 2'ye böl
		...
		*/
		...
		echo $mega['Data'][1]['SeqData']['dataString1'];